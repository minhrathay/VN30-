...
# ═══════════════════════════════════════════════════════════════════════════════
# PHẦN 0A: STATIONARITY TESTING (Kiểm tra Tính dừng)
# ═══════════════════════════════════════════════════════════════════════════════

def test_stationarity(series, significance=0.05, verbose=False):
    """
    ───────────────────────────────────────────────────────────────────────────────
    KIỂM TRA TÍNH DỪNG CỦA CHUỖI THỜI GIAN (Stationarity Testing)
    ───────────────────────────────────────────────────────────────────────────────
    
    Sử dụng 2 test bổ sung lẫn nhau:
    - ADF (Augmented Dickey-Fuller): H0 = Non-stationary
    - KPSS (Kwiatkowski-Phillips-Schmidt-Shin): H0 = Stationary
    
    Logic kết hợp:
    - Nếu ADF reject H0 VÀ KPSS không reject H0 → Chuỗi dừng
    - Ngược lại → Cần differencing
    ...
    try:
        ...
        # Reject H0 nếu p-value < significance → Series is stationary
        # ─────────────────────────────────────────────────────────────────────────
        ...
        # Reject H0 nếu p-value < significance → Series is non-stationary
        # ─────────────────────────────────────────────────────────────────────────
        ...
        # ─────────────────────────────────────────────────────────────────────────
        # Case 1: ADF rejects (stationary) + KPSS not reject (stationary) → Stationary
        # Case 2: ADF not reject + KPSS rejects → Non-stationary, need diff
        # Case 3: Both reject → Trend stationary (có xu hướng)
        # Case 4: Neither reject → Inconclusive, assume non-stationary
        
        if adf_reject and not kpss_reject:
            result['is_stationary'] = True
            result['recommended_diff'] = 0
        elif not adf_reject and kpss_reject:
            result['is_stationary'] = False
            result['recommended_diff'] = 1
        elif adf_reject and kpss_reject:
            # Trend stationary - có thể dùng hoặc detrend
            result['is_stationary'] = True  # Có thể dùng với trend component
            result['recommended_diff'] = 0
        else:
            # Inconclusive - giả định cần differencing
            result['is_stationary'] = False
            result['recommended_diff'] = 1
        
        if verbose:
            print(f"[Stationarity Test]")
            print(f"  ADF Statistic: {adf_statistic:.4f}, p-value: {adf_pvalue:.4f}")
            print(f"  KPSS Statistic: {kpss_statistic:.4f}, p-value: {kpss_pvalue:.4f}")
            print(f"  Conclusion: {'STATIONARY' if result['is_stationary'] else 'NON-STATIONARY'}")
            print(f"  Recommended differencing: d={result['recommended_diff']}")
            
    except Exception as e:
        if verbose:
            print(f"[Stationarity Test] Error: {e}")
        # Default to differencing if test fails
        result['recommended_diff'] = 1
    
    return result


def auto_difference(series, max_diff=2, significance=0.05):
    """
    ───────────────────────────────────────────────────────────────────────────────
    TỰ ĐỘNG DIFFERENCING ĐẾN KHI CHUỖI DỪNG
    ───────────────────────────────────────────────────────────────────────────────
    
    Áp dụng differencing lần lượt và kiểm tra stationarity sau mỗi lần.
    Dừng khi chuỗi đạt stationarity hoặc đạt max_diff.
    ...
    for d in range(max_diff + 1):
        stat_result = test_stationarity(current_series, significance)
...
# ═══════════════════════════════════════════════════════════════════════════════
# PHẦN 0B: GARCH VOLATILITY MODEL (Mô hình hóa Biến động)
# ═══════════════════════════════════════════════════════════════════════════════
...
def train_arimax(train_data, test_data, exog_cols=['Lag_Vol_1', 'RSI', 'ATR'], fast_mode=True):
    ...
    # ─────────────────────────────────────────────────────────────────────────────
    # BƯỚC 1: STATIONARITY TEST để xác định d (differencing order)
    # ─────────────────────────────────────────────────────────────────────────────
    stat_result = test_stationarity(train_endog, verbose=False)
    suggested_d = stat_result['recommended_diff']
    
    # ─────────────────────────────────────────────────────────────────────────────
    # BƯỚC 2: AUTO ORDER SELECTION với pmdarima
    # ─────────────────────────────────────────────────────────────────────────────
    try:
        ...
        auto_model = auto_arima(
            ...
            start_p=1, max_p=5,
            start_q=0, max_q=2,
            d=suggested_d,  # Sử dụng d từ stationarity test
            seasonal=False,
            stepwise=True,
        ...
        print(f"[ARIMAX] Auto-selected order: {best_order} (AIC: {auto_model.aic():.2f})")
        
    except ImportError:
        # Fallback nếu không có pmdarima
        print("[ARIMAX] pmdarima not installed, using default order (5,1,0)")
        best_order = (5, suggested_d, 0)
    except Exception as e:
        print(f"[ARIMAX] Auto order failed: {e}, using fallback")
        best_order = (5, suggested_d, 0)
    
    # ─────────────────────────────────────────────────────────────────────────────
    # BƯỚC 3: TRAINING VÀ PREDICTION
    # ─────────────────────────────────────────────────────────────────────────────
    ...
    if fast_mode:
        ...
        try:
            ...
            print(f"[ARIMAX] Forecast failed: {e}, using fallback")
        ...
        with warnings.catch_warnings():
            ...
            for t in range(len(test_data)):
                try:
                    ...
                    predictions.append(history_endog[-1])
...
def train_xgboost(train_data, test_data, n_days=14, use_tuning=True):
    ...
    Tham số:
        use_tuning: Nếu True, chạy Optuna optimization (chậm hơn nhưng tốt hơn)
        ...
        try:
            ...
            def objective(trial):
                ...
                for train_idx, val_idx in tscv.split(X_train):
                    ...
                    mape = mean_absolute_percentage_error(y_cv_val, preds)
            ...
            study.optimize(
                objective,
                n_trials=40,  # More trials than random search
                timeout=120,   # Max 2 minutes
                show_progress_bar=False
            )
            
            best_params = study.best_params
            print(f"[XGBoost] Optuna best params: {best_params}")
            print(f"[XGBoost] Optuna best MAPE: {study.best_value*100:.4f}%")
            ...
            # Fallback nếu không có Optuna - dùng RandomizedSearchCV
            print("[XGBoost] Optuna not installed, using RandomizedSearchCV fallback")
            ...
            print(f"[XGBoost] RandomizedSearch best params: {best_params}")
    else:
...
def walk_forward_validation(model_fn, df, initial_train_size=200, step_size=20, n_periods=5):
    """
    ───────────────────────────────────────────────────────────────────────────────
    ...
    - Window 1: Train[0:200], Test[200:220]
    - Window 2: Train[0:220], Test[220:240]
    - Window 3: Train[0:240], Test[240:260]
    - ...
    
    Đây là phương pháp đánh giá thực tế nhất cho time series vì:
    1. Không có look-ahead bias
    2. Mô phỏng quy trình dự báo thực tế
    3. Đánh giá khả năng thích ứng của model
    
    Parameters:
        model_fn: Function(train_data, test_data) -> predictions
        df: DataFrame hoàn chỉnh đã có features
        initial_train_size: Số mẫu ban đầu dùng để train
        step_size: Số mẫu mỗi bước test
        n_periods: Số kỳ validation
    ...
    Returns:
        dict: {
            ...
            'mape_std': float,
            'mape_per_fold': list,
            'predictions_all': list of (y_true, y_pred) tuples
        }
    ───────────────────────────────────────────────────────────────────────────────
    """
    results = {
    ...
    for fold in range(n_periods):
        ...
        try:
            ...
            results['predictions_all'].append((y_true, y_pred))
            
        except Exception as e:
            print(f"[Walk-Forward] Fold {fold+1} failed: {e}")
            continue
    
    if results['mape_per_fold']:
        results['mape_mean'] = np.mean(results['mape_per_fold'])
        results['mape_std'] = np.std(results['mape_per_fold'])
    else:
        results['mape_mean'] = 0
        results['mape_std'] = 0
...
def train_lstm(df, train_size, n_days=14):
    ...
    KIẾN TRÚC MỚI:
    - Lookback: 20 ngày (tăng từ 10 để bắt monthly cycles)
    - Features: Log_Ret, RSI_norm, ATR_norm, Volume_norm (4 features)
    - Attention: Self-Attention Layer giúp focus vào important time steps
    ...
    from tensorflow.keras.layers import Flatten, Add, Concatenate
    ...
    # Features list - added Momentum
    feature_cols = ['Log_Ret', 'RSI_norm', 'ATR_norm', 'Vol_Change', 'Momentum_5']
    ...
    # ─────────────────────────────────────────────────────────────────────────────
    # BƯỚC 5: XÂY DỰNG MÔ HÌNH LSTM VỚI ATTENTION MECHANISM
    # ─────────────────────────────────────────────────────────────────────────────
    ...
    attention_output = MultiHeadAttention(
        ...
        key_dim=32,         # Dimension for attention computation
        dropout=0.1
    )(lstm_out, lstm_out)  # Self-Attention: query = key = value
    
    # Residual connection + Layer Normalization
    attention_output = Add()([lstm_out, attention_output])
    attention_output = LayerNormalization()(attention_output)
    
    # Layer 2: LSTM để aggregate thông tin từ attention output
    lstm_out2 = LSTM(32, return_sequences=False)(attention_output)
    ...
    outputs = Dense(1, activation='tanh')(dense2)  # tanh vì log return thường trong (-1, 1)
    ...
    lstm_objects = {
        ...
        'test_predictions': pred_prices  # [ADDED] For walk-forward validation
    }
...
def forecast_future_lstm(lstm_objects, df, n_days=14):
    ...
    # ─────────────────────────────────────────────────────────────────────────────
    # BƯỚC 2: LẤY SEQUENCE CUỐI CÙNG (LOOKBACK ngày gần nhất)
    # ─────────────────────────────────────────────────────────────────────────────
    feature_data = df_lstm[feature_cols].values
    current_sequence = feature_data[-LOOKBACK:].copy()  # Shape: (LOOKBACK, n_features)
    ...
    # Lấy giá trị cuối của các features (mean-revert dần)
    last_rsi = df_lstm['RSI_norm'].iloc[-1]
    last_atr = df_lstm['ATR_norm'].iloc[-1]
    last_momentum = df_lstm['Momentum_5'].iloc[-1]
    ...
    for day in range(n_days):
        ...
        new_momentum = last_momentum * 0.8  # Momentum decay
        
        # Tạo feature vector mới (5 features)
        new_features = np.array([
            ...
            new_momentum    # Momentum decaying
        ])
