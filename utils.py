...
def load_data(filename='Dữ liệu Lịch sử VN 30.csv', uploaded_file=None, column_mapping=None):
    """Load and preprocess stock data from file or uploaded file
    
    Args:
        filename: Default CSV file to load
        uploaded_file: Streamlit uploaded file object
        column_mapping: Dict with keys 'Date', 'Close', 'Open', 'High', 'Low', 'Volume'
                        mapping to actual column names in the file
    """
    ...
    if column_mapping is not None:
        ...
        if column_mapping.get('Close'):
            rename_map[column_mapping['Close']] = 'Close'
        if column_mapping.get('Open'):
            rename_map[column_mapping['Open']] = 'Open'
        if column_mapping.get('High'):
            rename_map[column_mapping['High']] = 'High'
        if column_mapping.get('Low'):
            rename_map[column_mapping['Low']] = 'Low'
        if column_mapping.get('Volume'):
            rename_map[column_mapping['Volume']] = 'Vol'
